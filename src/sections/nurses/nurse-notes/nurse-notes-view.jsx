...
export default function NurseNotesView() {
  ...
  const loadNotes = useCallback(async () => {
    setLoading(true);
    try {
      // Simuler le chargement des notes infirmières
      const mockNotes = Array.from({ length: 20 }, (_, i) => ({
        ...
        type: ['observation', 'intervention', 'evolution'][Math.floor(Math.random() * 3)],
        titre: ['Observation clinique', 'Intervention effectuée', 'Évolution du patient'][
          Math.floor(Math.random() * 3)
        ],
        contenu: `Note détaillée concernant le patient. Observations importantes à noter pour le suivi.`,
      ...
      if (search) {
        ...
        filtered = filtered.filter(
          (n) =>
            ...
            n.infirmier.toLowerCase().includes(searchLower)
        );
      }
      ...
      showError('Erreur', 'Impossible de charger les notes infirmières');
      setNotes([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Helmet>
        <title> Notes Infirmières | Clinique </title>
      </Helmet>
      ...
      <Stack spacing={3}>
        <Box>
          <Typography variant="h4">Notes Infirmières</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            Consulter et gérer les notes infirmières sur les patients
          </Typography>
        </Box>
